...
def q104( conn ):
    ...
    sql = '''
          SELECT Date, ABS(Low-High) as Max_Diff_Intramensual
          FROM gold_price
          '''
    
    sql = '''
          SELECT *, ABS(Low-High) as Max_Diff_Intramensual
          FROM (
          gold_price INNER JOIN recessions ON
          strftime('%Y-%m-%d', gold_price.Date) == strftime('%Y-%m-%d', recessions.Date)
          );
    ...
    sql = '''
          SELECT Regime, AVG(ABS(Low-High)) as Max_Diff_Intramensual_Promedio
          FROM (
          gold_price INNER JOIN recessions ON
          strftime('%Y-%m-%d', gold_price.Date) == strftime('%Y-%m-%d', recessions.Date)
          )
...
def q108( conn ):
    ...
    sql = '''
          ...
          FROM gold_price
          GROUP BY Year
          HAVING Average_Close_Price > 21 OR YEAR >= "2008";
          '''
